...
class FKR_EigenPro(nn.Module):
    ...
    def evaluate(self, x_eval, y_eval, bs,
        ...
        if 'r' in metrics:
            eval_metrics['r'] = {idx: pearsonr(p_eval[:, idx], y_eval[:, idx])[0] for idx in range(y_eval.shape[-1])}
        if 'r2' in metrics:

            eval_metrics['r2'] = {idx: r2_score(y_eval[:, idx], p_eval[:, idx]) for idx in range(y_eval.shape[-1])}
    ...
    def fit(self, x_train, y_train, x_val, y_val, epochs, mem_gb,
        ...
        for epoch in epochs:
            ...
            if run_epoch_eval:
                ...
                #print("train error: %.5f\tval error: %.5f\tpearson r: %.5f (%d epochs, %.2f seconds)\t"% (tr_score['mse'], tv_score['mse'], tv_score['r'], epoch, train_sec))
                print(tr_score['mse'], tv_score['mse'])
                res[epoch] = (deepcopy(self.weight), tr_score['mse'], tv_score['mse'], tv_score['r'], tv_score['r2'])
                #"""
            #"""
            initial_epoch = epoch
